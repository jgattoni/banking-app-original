  ...
  try {
    ...
    const accounts = await Promise.all(
      banks?.map(async (bank: Bank) => {
        ...
        const accountsResponse = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/plaid/accounts`, {
          method: 'POST',
          headers: {
        ...
        const institution = await getInstitution({
          institutionId: accountData.item.institution_id!,
        });
...
export const getInstitution = async ({
  ...
  try {
    const institutionResponse = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/plaid/institution/${institutionId}`);
...
export const getTransactions = async ({
  ...
  try {
    ...
    while (hasMore) {
      const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/plaid/transactions`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
